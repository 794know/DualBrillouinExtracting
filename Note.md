...
对loss2进行一个更关注的拟合分支：
>监控为loss2单独增加一小段分支（如1-2层MLP），让网络有更强的能力拟合loss2相关特征。
...
1. 温度预期误差2，应变 * 0.05预期误差2，总误差4
2. 调整了学习率0.002，重新训练
3. 可变学习率的使用
4. 需要训练的早停机制。故添加早停机制，预计是连续20个epoch loss不怎么变则直接退出并保存

对应commit：20250609_1658_commit

#### 出现错误7

1. 早停语句过于生硬，应等待至epoch至少到30以后，so修改了

#### 训后笔记7

网络loss跑不低，温度RMSE在10左右，Strain RMSE为400左右，非常离谱的错误
